...
class SearchScreenState extends State<SearchScreen> {
  ...
  void initState() {
    ...
    if(_isLoggedIn) {
      Get.find<SearchGetController>().getSuggestedItems();
    }
    Get.find<SearchGetController>().getHistoryList();
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        if(Get.find<SearchGetController>().isSearchMode) {
          return true;
        }else {
          Get.find<SearchGetController>().setSearchMode(true);
          return false;
        }
      },
      child: Scaffold(
        ...
        body: SafeArea(child: Padding(
          ...
          child: GetBuilder<SearchGetController>(builder: (searchController) {
  ...
  void _actionSearch(bool isSubmit, String? queryText, bool fromHome) {
    if(Get.find<SearchGetController>().isSearchMode || isSubmit) {
      if(queryText!.isNotEmpty) {
        Get.find<SearchGetController>().searchData(queryText, fromHome);
      }else {
      ...
      List<double?> prices = [];
      if(!Get.find<SearchGetController>().isStore) {
        for (var product in Get.find<SearchGetController>().allItemList!) {
          prices.add(product.price);
        }
        prices.sort();
      }
      double? maxValue = prices.isNotEmpty ? prices[prices.length-1] : 1000;
      Get.dialog(FilterWidget(maxValue: maxValue, isStore: Get.find<SearchGetController>().isStore));
    }
  }
}
