    ...
    if(isSuccess) {
      if(Get.find<ParcelController>().paymentIndex == 1) {
        if(GetPlatform.isWeb) {
          ...
          String selectedUrl = '${AppConstants.baseUrl}/paypal/payment-mobile?order_id=$orderID&&customer_id=${Get.find<UserController>()
              .userInfoModel!.id}&&callback=$protocol//$hostname${RouteHelper.orderSuccess}?id=$orderID&status=';
          html.window.open(selectedUrl,"_self");
        } else{
          Get.offNamed(RouteHelper.getPaymentRoute(orderID, Get.find<UserController>().userInfoModel!.id, 'parcel', orderAmount, _isCashOnDeliveryActive,'paypal'));
        }
      }
      ...
      onPressed: parcelController.acceptTerms ? () {
        if(parcelController.distance == -1) {
          ...
          Get.find<OrderController>().placeOrder(PlaceOrderBody(
            ...
            doorbell: widget.pickedUpAddress.doorBell ?? '',
            floor: widget.pickedUpAddress.floor ?? '',
            discountAmount: 0, taxAmount: 0, parcelCategoryId: widget.parcelCategory.id.toString(),
            chargePayer: parcelController.payerTypes[parcelController.payerIndex], dmTips: _tipController.text.trim(),platform: platform!
